...
const Shorts = () => {
    ...
    videoRefs.current.forEach((video, index) => {
      if (video) {
        if (index === currentIndex) {
          video.play().catch(() => {});
          if (shorts[index]) {
            axios.post(`https://xenzys-api.sutirthasoor7.workers.dev/api/videos/${shorts[index].id}/view`);
          }
        } else {
          video.pause();
        }
  ...
  const fetchShorts = async () => {
    try {
      const response = await axios.get('https://xenzys-api.sutirthasoor7.workers.dev/api/videos?type=short');
  ...
  const handleLike = async (shortId) => {
    if (!liked[shortId]) {
      try {
        const res = await axios.post(`https://xenzys-api.sutirthasoor7.workers.dev/api/videos/${shortId}/like`);
  ...
  const handleComment = async (shortId) => {
    if (commentText.trim()) {
      try {
        const res = await axios.post(`https://xenzys-api.sutirthasoor7.workers.dev/api/videos/${shortId}/comment`, {
          text: commentText,
          username: 'You'
        });
